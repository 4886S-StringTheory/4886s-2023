...
void pre_auton(void) {
    ...
    B_SCRN.clearScreen();
    B_SCRN.drawRectangle(B_SCRN_X_MID / 8, 0, B_SCRN_X / 4, B_SCRN_Y_MID, green);
    B_SCRN.drawRectangle(B_SCRN_X_MID / 8 + B_SCRN_X / 2, 0, B_SCRN_X / 4, B_SCRN_Y_MID, red);

    B_SCRN.setFillColor(red);
    B_SCRN.setPenColor(black);
    B_SCRN.printAt(B_SCRN_X / 12 + B_SCRN_X / 2, B_SCRN_Y / 4, "LOCK");

    B_SCRN.setFillColor(green);
    ...
    if (auton_mode == SKILLS) inrtl.setRotation(90, ROT_DEG);
